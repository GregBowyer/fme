...
public class FunctionProcedureCallGraph {
    ...
    public boolean showCallGraph(boolean allProjectFiles, boolean showExternalCalls) {
        ...
        CM.getConfig().put("graph.ConnectionLine.Color",
                CM.getAsColor("gui.FunctionProcedureCallGraph.LineColor"));
        CM.getConfig().put("graph.ConnectionLine.MarkedColor",
                           CM.getAsColor("gui.FunctionProcedureCallGraph.MarkedLineColor"));
        CM.getConfig().put("graph.Textnode.BackgroundColor",
                           CM.getAsColor("gui.FunctionProcedureCallGraph.NodeColor.MODULE"));
            ...
            if (node.getSpecificType().equals("T_Name")) {
                if (ca.get(node.getValue()) != null) {
                    main.getMainDisplay().getVirtualSpaceManager()
                            .centerOnGlyph(ca.get(node.getValue()).getGlyphRepresentation(),
                                           main.getMainDisplay().getCamera(),
                                           500);
                }
            }
